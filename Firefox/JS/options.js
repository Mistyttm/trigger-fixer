...
chrome.storage.sync.get([
	...
	if (result.replaceWith !== undefined) {
        var refreshrate = result.replaceWith;
    }
    ...
    for (i = 1; i < replaceThisArr.length - 1; i++) {
		...
		var textnode = document.createTextNode(thisWord + " " + fillerPhrase + " " + withWord);
    ...
    function submitClicked() {
        ...
        chrome.storage.sync.set({'replaceThis': replaceThisCommaDel});
        chrome.storage.sync.set({'replaceWith': replaceWithCommaDel});
...
document.getElementById("search").addEventListener("onkeyup", list());
